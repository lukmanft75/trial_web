...
class FormElements extends Database {
    ...
    public function select_box($name,$title,$values,$selecteds=array(),$width,$height,$z_index=0,$maxselects=5,$title_height=12,$font_size=12,$title_color="#000",$border_color="#43579C",$contains_color="grey",$backcolor="white",$attr="",$class=""){
        $arr_elms = "";
        ...
        $return ='
            ...
            <div style="text-align:left;position:relative;width:'.($width+17).'px;height:5px;z-index:'.$z_index.';">
                <div onmouseover="try{ hiding_select_box(\'select_box_values_'.$name.'\'); } catch(e){}" style="display: flex;align-items: center;width:'.($width+17).'px;font-size:'.$font_size.'px;height:'.($title_height+1).'px;border:solid '.$border_color.' 1px;position:absolute;z-index:'.$z_index.';background-color:'.$backcolor.';">
	...
	public function select_window($name,$title,$value="",$tablename,$id_field,$caption_field,$window_filename = "win_default.php"){
		...
		if($value != "") $caption = $this->fetch_single_data($tablename,$caption_field,array($id_field => $value));
