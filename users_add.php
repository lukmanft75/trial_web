...
<?php
	if(isset($_POST["save"])){
		...
		if($inserting["affected_rows"] >= 0){
			...
			javascript("window.location='indottech_teams_list.php';");
		} else {
	...
	$parents 			= $db->fetch_select_data("users","id","concat(email,' -- ',name)",["forbidden_chr_dashboards" => $__main_menu_id],[],"",true);
...
<?=$f->start();?>
	...
	<?=$t->end();?>
	<?=$f->input("save","Save","type='submit'");?> <?=$f->input("back","Back","type='button' onclick=\"window.location='indottech_teams_list.php';\"");?>
<?=$f->end();?>
